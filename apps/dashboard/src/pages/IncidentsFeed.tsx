...
import { AlertTriangle, Clock, CheckCircle, Search, Database, Shield, Wifi, HardDrive, Zap, Bug, ChevronRight, BookOpen, Bell, Check, RefreshCw, ChevronDown, CheckSquare, Square, X, Download } from 'lucide-react';
import { api } from '../api';
...
export default function IncidentsFeed() {
  ...
  const quickAction = async (e: React.MouseEvent, incId: string, status: string, label: string) => {
    e.stopPropagation();
          ...
          return (
            <div key={inc.id} onClick={() => navigate(`/incidents/${inc.id}`)}
              ...
              <div className="flex items-center gap-4">
                <button onClick={(e) => toggleSelect(e, inc.id)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-[12px]" style={{ color: 'var(--apple-text-tertiary)' }}>
                    ...
                    <span>{timeAgoMap.get(inc.createdAt) || ''}</span>
                    {inc.pagerduty && (
